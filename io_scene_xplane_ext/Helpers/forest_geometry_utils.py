...
def get_stiffness_vertex_group(obj):
    """
    Get or create a vertex group that contains "stiffness" (but not "edge").
    Args:
        obj (bpy.types.Object): The Blender object to get the vertex group from.
    Returns:
        bpy.types.VertexGroup: The vertex group containing "stiffness" or the created "w_stiffness" group.
    """
    if obj.type != 'MESH':
        raise TypeError("Object must be a mesh")
    
    # Search for existing vertex group containing "stiffness" but not "edge"
    for vg in obj.vertex_groups:
        if "stiffness" in vg.name.lower() and "edge" not in vg.name.lower():
            return vg
    
    # If not found, create one
    return obj.vertex_groups.new(name="w_stiffness")

def get_edge_stiffness_vertex_group(obj):
    """
    Get or create a vertex group that contains both "edge" and "stiffness".
    Args:
        obj (bpy.types.Object): The Blender object to get the vertex group from.
    Returns:
        bpy.types.VertexGroup: The vertex group containing both "edge" and "stiffness" or the created "w_edge_stiffness" group.
    """
    if obj.type != 'MESH':
        raise TypeError("Object must be a mesh")
    
    # Search for existing vertex group containing both "edge" and "stiffness"
    for vg in obj.vertex_groups:
        if "edge" in vg.name.lower() and "stiffness" in vg.name.lower():
            return vg
    
    # If not found, create one
    return obj.vertex_groups.new(name="w_edge_stiffness")

def get_phase_vertex_group(obj):
    """
    Get or create a vertex group that contains "phase".
    Args:
        obj (bpy.types.Object): The Blender object to get the vertex group from.
    Returns:
        bpy.types.VertexGroup: The vertex group containing "phase" or the created "w_phase" group.
    """
    if obj.type != 'MESH':
        raise TypeError("Object must be a mesh")
    
    # Search for existing vertex group containing "phase"
    for vg in obj.vertex_groups:
        if "phase" in vg.name.lower():
            return vg
...
def create_for_obj_from_draw_call(vertices, indicies, name):
    ...
    for i, vertex in enumerate(vertices):
        if vertex.stiffness > 0:
            stiffness_vg.add([i], vertex.stiffness, 'REPLACE')
        if vertex.edge_stiffness > 0:
            edge_stiffness_vg.add([i], vertex.edge_stiffness, 'REPLACE')
        if vertex.phase > 0:
            phase_vg.add([i], vertex.phase, 'REPLACE')
...
def get_for_draw_call_from_obj(obj):
    ...
    try:
        ...
        # Get the vertex groups for extracting weight data
        stiffness_vg = get_stiffness_vertex_group(obj)
        edge_stiffness_vg = get_edge_stiffness_vertex_group(obj)
        phase_vg = get_phase_vertex_group(obj)
        ...
        for tri_idx, tri in enumerate(xp_triangles):
            # Get mesh vertex indices for this triangle
            ...
            for v_idx, vert in enumerate([v1, v2, v3]):
                mesh_v_idx = mesh_v_indices[v_idx]
                # Get weights from vertex groups, default to 0 if vertex is not in group
                try:
                    vert.stiffness = stiffness_vg.weight(mesh_v_idx)
                except RuntimeError:
                    vert.stiffness = 0
                try:
                    vert.edge_stiffness = edge_stiffness_vg.weight(mesh_v_idx)
                except RuntimeError:
                    vert.edge_stiffness = 0
                try:
                    vert.phase = phase_vg.weight(mesh_v_idx)
                except RuntimeError:
                    vert.phase = 0
