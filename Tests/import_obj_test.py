...
def test(test_dir):
    ...
    test_helpers.add_test_category("Import Object Tests")
    ...
    try:
        ...
        test_helpers.add_test_name("Import Object Test Error")
        test_helpers.append_test_fail("Error finding .blend files: " + str(e))
        return

    #For each blender file open it, import the associated .obj (same base file name), then compare the two collections in the scene
    for obj_file in import_objs:
        try:
            ...
            failed_count += 1
            test_helpers.append_test_fail(os.path.splitext(os.path.basename(obj_file))[0])

    test_helpers.add_test_name("Object Importer Summary")
    test_helpers.append_test_results(
        failed_count == 0,
