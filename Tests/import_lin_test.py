...
def test(test_dir):
    relative_test_dir = "Import Line Tests"
    ...
    try:
        ...
        test_helpers.add_test_name("Import Line Test Error")
        test_helpers.append_test_fail("Error finding .blend files: " + str(e))
        return
    
    test_helpers.add_test_category("Import Line Tests")
    ...
    for lin_file in import_lin:
        ...
        try:
            ...
            print(f"Error processing {lin_file}: {str(e)}")
            failed_count += 1
            test_helpers.append_test_fail(os.path.splitext(os.path.basename(lin_file))[0])

    test_helpers.add_test_name("Line Importer Summary")
    test_helpers.append_test_results(
        failed_count == 0,
